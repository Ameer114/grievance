...
import DepartmentCRUDClient from './DepartmentCRUDClient';
import { Card } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { FileText, Building } from 'lucide-react';

export default async function AdminDepartmentsCRUDPage() {
  ...
  // 3. Fetch departments using Admin client to bypass RLS select
  const { data: departmentsData } = await adminSupabase
    .from('departments')
    .select('*')
    ...
    <div className="flex-1 flex flex-col bg-slate-50/50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          <DepartmentCRUDClient initialDepartments={departments} />
        </div>
      </div>
    </div>
  );
